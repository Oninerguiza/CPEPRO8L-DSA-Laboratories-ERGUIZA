...
If the middle element is greater than the target, the target can only be located in the left half of the current range:
...
The complete call sequence is therefore:
...
This condition is important because it handles the situation where the target does not exist in the array. Once the search range becomes empty, the function stops making recursive calls and returns -1.
...
For example, if the recursive function continued making calls without reaching a stopping condition:
...
The search only requires two recursive calls because the Binary Search algorithm eliminates approximately half of the remaining elements during each step.
...
The space complexity differs between iterative and recursive implementations.
...
An iterative implementation uses a loop instead of recursive function calls. It only requires a constant number of additional variables, so its auxiliary space complexity is:
...
The recursive version is therefore less space-efficient than the iterative version because of the memory required by the call stack.
